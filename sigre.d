/++ dub.json: {
    ...
    "dependencies": {
        "dffdd": {
            "repository": "git+https://github.com/k3kaimu/dffdd.git",
            "version": "a5aae3926356a028b3bbb319bf2d349a6e08255d"
        }
    },
...
/// $ sigre 1000 2000 PHammLS 32 @ <Basis Function Type> [Basis Functions Paramters] % Orthonormalizer # 入力を直交化する
/// $ sigre 1000 2000 PHammLS 32 @ OFDMUpSampler 64 16 8 % <Basis Function Type> [Basis Functions Paramters] % OFDMDownSampler 64 16 8 0,1,-1 # 入力信号がFFTサイズ64，CP長16のOFDM信号だと仮定して，それを8倍にオーバーサンプリングしてから基底関数にかけて，1/8倍にデシメーションして元に戻してからキャンセラに入れる
import std;
import dffdd;
...
version(SIGRE_TEST_MODE) static this()
{
    ...
    auto regen = new PHammLSRegenerator!C(xs, ys, ["1"], new NullBBB!C());
...
version(SIGRE_TEST_MODE) static this()
{
    ...
    auto regenImpl = new PHammLSRegenerator!C(xs, ys, ["4"], makeBBB!C(["BFList", "X[1,0]", "X[2,0]"]));
...
{
    T failureMsg(T)(lazy T value, lazy string msg)
    {
        T dst;
        try {
            dst = value();
...
{
    this(uint nFFT, uint nCP, uint nOS, int[] bandSelects)
    {
        _nFFT = nFFT;
        _nCP = nCP;
        _nOS = nOS;
        _bands = bandSelects.dup;
    ...
    C[][] generate(in C[][] xs)
    {
        C[][] dst = new C[][](xs.length * _bands.length);

        size_t cnt = 0;
        foreach(i; 0 .. xs.length) foreach(j; 0 .. _bands.length) {
            dst[cnt] = genImpl(xs[i], _bands[j]);
            ++cnt;
        }
  ...
  private:
    size_t _nFFT, _nCP, _nOS;
    immutable(int)[] _bands;
    FFTWObject!Cpx _fftH, _fftL;


    C[] genImpl(C)(in C[] xs, int bandIndex)
    {
        ...
        foreach(i; 0 .. xs.length / ((_nFFT + _nCP)*_nOS) ) {
            ...
            if(bandIndex == 0) {
                ...
                _fftL.inputs!F[0 .. _nFFT] = _fftH.outputs!F[_nFFT / 2 + _nFFT * (bandIndex-1) .. _nFFT / 2 + _nFFT * bandIndex];
            } else  {   /* bandIndex < 0 */
                _fftL.inputs!F[0 .. _nFFT] = _fftH.outputs!F[$ - _nFFT / 2 - _nFFT * (-bandIndex) .. $ - _nFFT / 2 - _nFFT * (-bandIndex - 1)];
            }
            _fftL.ifft!F();
...
version(SIGRE_TEST_MODE) static this()
{
    ...
    C[][] ys = new OFDMDownSampler!C(2, 1, 8, [0, 1, -1]).generate(new OFDMUpSampler!C(2, 1, 8).generate([xs]));
    foreach(i; 0 .. xs.length) {
        assert(approxEqualC(xs[i], ys[0][i]));
        assert(approxEqualC(C(0), ys[1][i]));
        assert(approxEqualC(C(0), ys[2][i]));
    }
}


final class RemoveZeroBasis(C) : IBasisBuildingBlock!C
{
    this(double thr)
    {
        _thr = thr;
    ...
    {
        if(!_isInit) {
            _isRemove = new bool[xs.length];
            foreach(i; 0 .. xs.length) {
                immutable pwr = xs[i].map!sqAbs.sum / xs[i].length;
                if(pwr <= _thr)
                    _isRemove[i] = true;
            }
...
version(SIGRE_TEST_MODE) static this()
{
    ...
    C[][] ysremove = new RemoveZeroBasis!C(1e-10).generate(ys);
    assert(ysremove.length == 1);
    foreach(i; 0 .. xs.length)
        assert(approxEqualC(xs[i], ysremove[0][i]));
}
...
/**
StackMachine Operators...

Example:
StackMachine x/0 x/2                            # 入力信号の0次元目と2次元目のみを抜き出す
StackMachine X x/0 x/1 + x/2 x/3 + *            # 入力信号に(x[0]+x[1])*(x[2]+x[3])という信号を追加する
StackMachine X x/0 delay/1 x/1 delay/2 *        # x[0]を1サンプル遅延したものをx[1]を2サンプル遅延したものの積を追加する
StackMachine x/0 seq/0/0.1 rexpi *              # x[0][n] に exp(j0.1n)を乗算する

Operator:
+ X ... 入力のすべての次元の信号を順番にスタックに入れる．スタックのトップは最終次元の信号になる．
+ x/i ... i番目の次元の入力信号をスタックに入れる
+ +,*,-,/ ... トップの信号x_1とその次の信号x_2を演算してスタックに戻す．たとえば-であればx_2 - x_1である．
+ c/a/b ... 全要素がa + jbな複素数の信号をトップに入れる
+ seq/a/b ... 実数a,bについてn=0,1,...番目の成分がa+b*nになる信号を生成してトップに入れる
+ dup ... トップを複製してスタックに入れる
+ drop ... トップを捨てる
+ all/op ... opをスタックにあるすべての信号に作用させる．opは単体で使うとスタックトップにのみ作用する操作．たとえばall/sinはすべてのスタックにあるすべての信号にsinを適用する
+ topN/n/op ... opをスタックのトップn個の信号に作用させる．opは単体で使うとスタックトップのみに作用する操作．
+ rotU/m ... スタックのトップm個を上方向に1個回転させる
+ rotD/m ... スタックのトップm個を下方向に1個回転させる
+ rotUN/n/
+ delay/n ... トップの信号をnサンプル遅延する
+ pow/n/m/k ... トップの信号をx^n (x^{*})^m |x|^kにする
+ real ... トップの信号の実部のみを取り出す
+ imag ... トップの信号の虚部のみを取り出す
+ conj ... トップの信号をx^{*}にする
+ base64c/ABCX.... ... base64でデコードしたバイナリ列をComplex!floatの配列としてスタックトップに入れる
+ zbase64c/ABCX... ... base64でデコードしたバイナリ列をzlibで伸長してからComplex!floatの配列としてスタックトップに入れる
+ base64z/ABCX.... ... base64でデコードしたバイナリ列をComplex!doubleの配列としてスタックトップに入れる
+ zbase64z/ABCX... ... base64でデコードしたバイナリ列をzlibで伸長してからComplex!doubleの配列としてスタックトップに入れる
+ sin/cos/tan/sinh/cosh/tanh/asin/acos/atan/asinh/acosh/atanh/log/log10/sqrt/exp ... 複素数を引数に取る関数を作用する
+ rsin/rcos/rtan/rsinh/rcosh/rtanh/rasin/racos/ratan/rasinh/racosh/ratanh/rlog/rlog10/rsqrt/rexp/rexpi ... 信号の実部だけを取り出して，実数を引数に取る関数に作用させる
+ 
*/
// final class StackMachineBBB(C) : IBasisBuildingBlock!C
// {
//     this(string[] params)
//     {
//         _parsed = parseBasisIndexList(params);
    ...
    while(params.length != 0) {
        ...
        switch(curr.name) {
            ...
            case "OFDMDownSampler":
                enforce(curr.params.length == 3 || curr.params.length == 4, "[OFDMDownSampler] Given parameters %s are ignored.".format(curr.params[4 .. $]));
                ...
                int[] bandSelects = [0];
                if(curr.params.length >= 4) {
                    bandSelects = curr.params[3].split(",").map!(a => a.to!int.failureMsg!(a => std.math.abs(a)*2 < nOS)("[OFDMDownSampler] The band selectors must satisfy 'a * 2 < %s'.".format(nOS))).array();
                }

                ret = new ChainedBBB!C(ret, new OFDMDownSampler!C(nFFT, nCP, nOS, bandSelects));
                break;

            case "RemoveZeroBasis":
                enforce(curr.params.length <= 1, "[RemoveZeroBasis] Given parameters %s are ignored.".format(curr.params[1 .. $]));
                double thr = 1e-10;
                if(curr.params.length == 1) {
                    thr = curr.params[0].to!double.failureMsg!"a >= 0"("[RemoveZeroBasis] The first argument must satisfy 'a >= 0'.");
                }

                ret = new ChainedBBB!C(ret, new RemoveZeroBasis!C(thr));
                break;
